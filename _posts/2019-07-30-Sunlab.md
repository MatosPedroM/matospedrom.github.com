...
Dealing with weather data is always challenging due to variability and, in some cases, inconsistent sensor readings.
...
For a baseline model we defined a simple Liner Regression resulting in R2 of 0.75 (test set).

Then we built a Random Forest model were we achieved an R2 of 0.89 (test set).
